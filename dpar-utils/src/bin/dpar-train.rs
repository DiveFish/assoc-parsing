...
fn main() {
    ...
    let (train_labels, train_embeds, train_lookup_inputs, train_non_lookup_inputs) =
    ...
    let (validation_labels, validation_embeds, validation_lookup_inputs, validation_non_lookup_inputs) =
        collect_data(&config, &vectorizer, reader).or_exit("Tensor collection failed", 1);
...
fn train(
    ...
    let train_fun: Box<Fn(_, _, _, _, _, _, _, _, _, _) -> Result<_, _>> =
    ...
    S: SerializableTransitionSystem,
{
    ...
    for (labels, embeds, lookup_inputs, non_lookup_inputs) in izip!(labels.iter(), embeds.iter(), lookup_inputs.iter(), non_lookup_inputs.iter()
        ) {
        let batch_perf = if is_training {
            model.train(embeds, lookup_inputs, non_lookup_inputs, labels, lr)
        } else {
            model.validate(embeds, lookup_inputs, non_lookup_inputs, labels)
        };
...
) -> Result<(Vec<Tensor<i32>>, Vec<Tensor<f32>>, Vec<LayerTensors<i32>>, Vec<Tensor<f32>>), Error>
where
    R: BufRead,
{
