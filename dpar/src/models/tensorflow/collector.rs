...
impl<'a, T> TensorCollector<'a, T> where
    T: TransitionSystem,
{
    ...
    /// of the last batch.
    pub fn into_data(mut self) -> (Vec<Tensor<i32>>, Vec<Tensor<f32>>, Vec<LayerTensors<i32>>, Vec<Tensor<f32>>) {
        self.resize_last_batch();

        (self.labels, self.embeds, self.lookup_inputs, self.non_lookup_inputs)
    }
...
mod tests {
    ...
    use features::{
        self, AddressedValues, DependencyEmbeddings, InputVectorizer, LayerLookups, Lookup, MutableLookupTable,
    };
    ...
    fn collect_zero() {
        ...
        let (labels, embeds, lookup_inputs, non_lookup_inputs) = collector.into_data();
    ...
    fn collect_one_pmi() {
        ...
        collector
            .collect(&StackProjectiveTransition::Shift, &state)
            .unwrap();
        let (_labels, _embeds, _lookup_inputs, non_lookup_inputs) = collector.into_data();
        ...
        // Check batch contents.
        assert_eq!(&*non_lookup_inputs[0], &[0.0, 0.0, 0.0, 0.0]);
        assert_eq!(&*non_lookup_inputs[1], &[0.0, 0.0, 0.5, 0.5]);
    }
    ...
    fn collect_two_pmis() {
        ...
        assert_eq!(
            &*non_lookup_inputs[2],
            &[1.0, 0.5, 0.0, 0.0, 0.5, 0.5, 0.5, 0.5]
        );
    }
