    ...
    T: TransitionSystem,
{
    ...
    pub fn predict(
        ...
        let logits = self.logits(embeds_tensor, lookup_input_tensors, non_lookup_input_tensors);
    ...
    fn logits(
        ...
        add_to_args(&mut args, &self.lookup_layer_ops, &lookup_input_tensors, &self.non_lookup_layer_op, &non_lookup_input_tensors);
    ...
    pub fn train(
        ...
        self.validate_(args, embeds, lookup_input_tensors, non_lookup_input_tensors, targets)
    }
...
fn add_to_args<'l>(
    ...
    for (layer, layer_op) in &layer_ops.0 {
        ...
        args.add_feed(&layer_op.0, 0, &lookup_input_tensors[layer]);
    }
...
mod tests {
    ...
    use features::{AddressedValues, DependencyEmbeddings, InputVectorizer, Layer, LayerLookups};
    ...
    fn load_graph_test() {
        ...
        let vectorizer = InputVectorizer::new(
            ...
            Vec::new(),
            DependencyEmbeddings::from(R2VEmbeddings::new(
                None,
